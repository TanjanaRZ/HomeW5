...
while rocks > 0:
    if hod == 0:
        tc = int(input("Ход первого игрока. Сколько? "))
        if tc > 0 and tc <= 28:
            rocks = rocks - tc
            print("Первый игрок взял " + str(tc) + ", осталось: " + str(rocks))
            hod = 1
    elif hod == 1:
        pltc = int(input("Ход второго игрока. Сколько? "))
        if pltc > 0 and pltc <= 28:
            rocks = rocks - pltc
            print("Второй игрок взял " + str(pltc) + ", осталось: " + str(rocks))
            hod = 0
        else:
